...
export const getBackendUrl = (): string => {
  ...
  return "https://agentplatform-6rol5.ondigitalocean.app";
};
...
export const getBackendEndpointUrl = (endpoint: string): string => {
  const mappedEndpoint = API_ENDPOINT_MAPPINGS[endpoint] || endpoint;
  return `${API_PROXY_CONFIG.backendUrl}${mappedEndpoint}`;
};
